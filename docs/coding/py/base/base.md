...
#### 常用方法
* `dict.keys/valuse/items()` 获取关于字典键 / 值 / 键值对元组的迭代, 可用于 `for` 循环
* `dict.copy()` 获取字典对象的浅复制
* `<key> in dict` 判断字典中是否有键 `key` (注意字典的 `in` 用于判断键)
...
* lambda 表达式可以没有任何参数, 也可有任意数量的参数, 包括两种[不定长参数](#不定长参数)
* lambda 表达式的函数体只能有一行, 且这一行的执行结果将作为表达式的返回值
* lambda 表达式的作用域与
* 可将 lambda 表达式作为一个回调函数赋给其他变量或作为其他函数参数
* 由于 lambda 表达式的特点, 其更多用于发挥包装复杂函数的作用, 类似 C++ 的 `std::bind`
* 在 lambda 表达式中不允许赋值

### 作用域
1. 函数内的局部变量无法被外部访问
1. 函数可直接访问外部的全局变量
1. 函数内也可以定义内部函数, 内部的函数只能在该函数内被访问, 但可以作为回调函数返回或作为参数
1. 如果要使函数内的局部变量被外部访问, 需要使用 `global <变量定义>`
