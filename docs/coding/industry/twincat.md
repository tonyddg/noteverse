...
其次要在 `VAR_INPUT` 内定义输入变量 , 输入变量将在初始化时由外部传入的参数确定  
然后要在 `VAR_OUTPUT` 内定义输出变量, 输出变量反映到外部变量上, 从而被读取    
最后在 `VAR` 定义局部变量  

首先在变量声明区定义功能块的实例 `[val] : [fb]`  
然后通过 `[val]([in1] := [val_in1], ..., [out1] => [val_out1], ...);` 初始化功能块  
最后可使用 `[val].[met](...)` 的方式调用功能块的方法 
