...
* 使用密码连接 `ssh <user>@<ip>`
    ...
    * `user` 登录使用的用户名
    * 登录到 Windows 时注意
        * `user` 可通过服务端的 `whoami` 命令确定
        * 如果使用 Microsoft 账户, 则登陆密码为对应账户密码, 不是 pin
* 生成密钥对 `ssh-keygen -t <method> [-C <doc>]`
    * `method` 生成算法, 推荐使用 `ed25519`
    * `doc` 关于密钥对的说明
    * 执行该命令后, 将会询问密钥对文件名称与密钥对保护密码
    * 命令完成后, 将在保存位置生成私钥文件与 `.pub` 的公钥文件 (建议在 ssh 配置文件夹执行命令)
* 向远程服务器发送公钥 `ssh-copy-id -i <pub> <user>@<ip>`
    * `pub` 生成的公钥文件
    * `user`, `ip` 与密码链接时相同
    * 将公钥文件内容添加到服务端的 `authorized_keys` 可以实现同样的效果
* 通过密钥对方式连接 `ssh -i <private> <user>@<ip>`  
    * `private` 生成的私钥文件
    * 连接时需要输入公钥文件的保护密码
* 强制密钥对方式连接设置 (见视频介绍)
