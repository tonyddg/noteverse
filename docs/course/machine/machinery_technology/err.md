...
当 $\Delta_{bc}$ 与 $\Delta_{jw}$ 有多个独立的来源时, 则可以分开分别计算 $d_{dwi}$, 然后相加
...
仅当 $C_p>1$ 表明工序中绝大部分的工件均能满足公差 $T$ 的要求, 工序能力足够  
当 $C_p<1$ 表明工序能力不足, 不合格品无法壁面
