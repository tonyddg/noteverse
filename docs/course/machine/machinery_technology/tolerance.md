...
* 过渡配合
当最大间隙 $X_{\max}>0$ 且最大过盈 $Y_{\max}<0$, 为过渡配合  
此时轴公差带与孔公差带相互重叠或包含  
用于孔与轴的定心连接
...
#### 公差等级选择
* 联系工艺
    * 公称尺寸 $\le 500$, 公差等级等于或低于 (精度高于) $IT8$ 时, 孔的公差等级比轴低一级, 如 $H8/f7$
    * 公称尺寸 $>500$ 或公差等级等于或高于 (精度第于) $IT9$ 时, 孔轴的公差等级相同, 如 $H9/d9$
* 联系配合
    * 过渡配合 (定心) 与过盈配合 (防止涨破) 不允许[配合公差](#配合公差)过大, 因此精度应该较高 (选用较低的公差等级)
    * 间隙配合允许最小间隙小时, 精度应该较高 (选用较低的公差等级), 如 $H6/g5$ (注意基准 $g$ 的 $es$ 更接近零线, 因此间隙小)   
    否则可采用较低的精度 (选用较高的公差等级), 如 $H10/a10$ (注意基准 $a$ 的 $es$ 更远离零线, 因此间隙大)
* 其他原则
    * 轴承 / 齿轮的公差与其精度等级有关
    * 类比选用
...
* 确定公差等级
    ...
    1. 平均分配配合公差, 初步确定轴孔的公差 $T_H=T_S=T_f / 2$
    1. 查表确定最接近的公差等级, 当公差等级低于 $IT8$ (精度高于 $IT8$) 时, 孔的公差比轴高一级 (孔精度低于轴, 即选取==相邻两侧==的等级)
* 确定基本公差
...
#### 公差带计算
量规的公差带宽度 $T_1$ 与偏移量 $Z_1$ 则根据零件的公差等级与公称尺寸确定 (查表) 

* 通规的极限偏移使用 $T_s,T_i$ 表示, 止规的极限偏移使用 $Z_s,Z_i$ 表示
* 通常公差带的一边与极限尺寸相切
* 通过还需要偏移, 其偏移量 $Z_1$ 的含义是公差带==中心线==相对极限尺寸==的位置==, 因此以孔为例 
$$T_s=Z_1+\frac{T_1}{2}+EI\quad T_i=Z_1-\frac{T_1}{2}+EI$$

### 通用计量器具的检验
使用通用计量器具时, 为了避免误收, 则会在零件的极限尺寸范围内, 向内移动一个安全裕度 $A$, 作为验收时的极限尺寸  
安全裕度与公称尺寸以及公差等级有关 (一般为公差带宽度的 $10\%$)
...
* 最大实体要求
    ...
    * 使用最大实体实效尺寸为边界的最大实效边界
* 满足最大实体要求的条件
$$\begin{split}
&d_{fe}=d_{a\max}+f\le d_{\max}+t_g=d_{MMVS},\;d_{\min}\le d_{a}\le d_{\max}\\
&D_{fe}=D_{a\min}-f\ge D_{\min}-t_g=D_{MMVS},\;D_{\min}\le D_{a}\le D_{\max}
\end{split}$$
* 使用最大实体要求  
