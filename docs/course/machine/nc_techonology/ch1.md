...
以下三个指令均为模态指令, 且相互替代
    * `G40` 取消刀具半径补偿
    * `G41 D__` 左刀补, 此时刀具在工件的左侧, 参数 `D` 为刀补号, 具体值需要通过 MDI 输入
...
复合循环车削即根据给出的 `ns~nf` 程序段规定的理论轮廓, 按指定的进到方向进行循环粗加工, 得到留下余量的轮廓等待下一步的粗加工  
粗加工中, 从起点开始, 往进刀方向深入切削深度 $\Delta d$, 然后沿垂直方向进给, 加工到余量处后, 退刀 $e$ 并回到起点, 再深入到下一层切削层 $2\Delta d$ 继续切削  
图中点 $A$ 为循环指令起点, 一般为工件顶点向外偏移  
$B,C$ 为理论轮廓外沿的两点, ==且点 $B$ 的 Z 坐标, 点 $C$ 的 X 坐标与起点 $A$ 相同==, 因此不一定与理论轮廓的两端点重合

* `G71 U__ R__ P__ Q__ X__ Z__` 径向进刀粗车复合循环指令
...
* `G72 U__ R__ P__ Q__ X__ Z__` 端面进刀粗车复合循环指令
    * 参数含义与 `G72` 相同
...
* `G73 U__ W__ R__ P__ Q__ X__ Z__` 仿形粗车复合循环指令
    * `U` X 方向的总进到量 $\Delta i$
    * `W` Z 方向的总进到量 $\Delta w$
    * `R` 循环加工次数
    * `P` 精加工开始程序段的顺序号
...
以下三个指令均为模态指令, 且相互替代
    * `G49` 取消刀具长度补偿
    * `G43 H__` 正向偏置, 此时该刀具长度==长与==建立坐标系的==对刀刀具==, 刀补值为设置刀具的长度减去对刀刀具, 参数 `H` 为偏置号, 具体值需要通过数控系统输入
...
* 在实际程序编写中
    ...
    1. 推荐理论路径的第一步与最后一步走直线, 且理论路径不一定要封闭, 且该方法只适用于加工单个封闭轮廓, 对于多个轮廓应当设置多次切削
    1. 设置下一工步的刀补, 重复上一步, 直到加工完成
    1. 由于参数 `D` 具有模态性, 因此可以在调用子程序前通过 `G41 D__` 规定一个刀补号, 之后进入子程序直接调用 `G41/40` 就可以启用与关闭刀补 
...
N02 G00 X0Y0
N03 T__ M06; 
N04 G44 H__ [G54 M03 S__ F__]; 
```

* `N01~N02` 用于为换刀做准备, 关闭主轴与冷却液, 抬刀取消长度补偿, 回到原点
* `T__` 即[刀具选择指令](#常用地址符及含义), 选择新刀具的刀具号
* `M06` 即[换刀准备](#其他控制指令), 需要与换刀指令出现在同一程序段中
* `G44 H__` 在换刀后应立即设置刀具的[长度补偿](#铣削刀具补偿)
* `G54 M03 S__ F__` 该部分用于铣削程序的开始 (即使刀具已经安装在主轴, 也最好在程序开始换刀)
