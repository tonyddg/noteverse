...
* $\rho c\frac{\partial t}{\partial\tau}$ 表示微元体热力学能 (温度) 变化而被吸收的热量  
...
根据[一维无热源稳态导热的特性](#一维无热源稳态导热的特性) ($q$ 在壁面内处处相等) 定性分析
...
公式中的分子体现了物体升高单位温度所吸收的热量 (即热容 $V\rho c$)  
...
* $h$ 为外界的表面传热系数
* $\lambda$ 为物体的导热系数
* ==$L=\frac{V}{A}$ 为物体==的特征长度, 注意 $A$ 为有效换热面积
* $M$ 为形状修正系数
    * 对于厚度为 $\delta$ 的==无限大==平面壁==双面换热==  
