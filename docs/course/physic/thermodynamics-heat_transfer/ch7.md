...
### 特殊一维导热的热流量
#### 圆筒导热热阻
通过变换为圆柱坐标系, 在 $r$ 方向上有导热微分方程
$$\frac{\mathrm{d}}{\mathrm{d}x}(r\frac{\mathrm{dt}}{\mathrm{d}r})=0$$

有边界条件
$$t(r_1)=t_1,t(r_2)=t_2$$

求解可得热流密度满足 (假设 $r_2>r_1$)
$$q(r)=\frac{\lambda\cdot(t_1-t_2)}{r\ln(r_2/r_1)}$$

注意, 由于圆柱截面面积不断变化, 因此==各个圆柱截面 $q$ 不相等, 随半径坐标 $r$ 增大而减小==

乘上圆柱面积 $A=2\pi rl$ 后 ($l$ 为圆筒长度)  
可得总热流量满足
$$\Phi=\frac{2\pi l\lambda\cdot(t_1-t_2)}{\ln(r_2/r_1)}$$
...
#### 球壁导热热阻
通过变换为球坐标系, 在 $r$ 方向上有导热微分方程
$$\frac{\mathrm{d}}{\mathrm{d}x}(r^2\frac{\mathrm{dt}}{\mathrm{d}r})=0$$

有边界条件
$$t(r_1)=t_1,t(r_2)=t_2$$

求解可得热流密度满足 (假设 $r_2>r_1$)
$$q(r)=\frac{\lambda\cdot(t_1-t_2)}{r^2(\frac{1}{r_1}-\frac{1}{r_2})}$$

注意, 由于球面面积不断变化  
乘上球面积 $A=4\pi r^2$ 后    
可得总热流量满足
$$\Phi=\frac{4\pi\lambda\cdot(t_1-t_2)}{\frac{1}{r_1}-\frac{1}{r_2}}$$

圆筒壁的导热热阻为
$$R=\frac{\frac{1}{r_1}-\frac{1}{r_2}}{4\pi l\lambda}$$
...
### 内热源圆柱导热
通过变换为圆柱坐标系, 在 $r$ 方向上有导热微分方程
$$\frac{\mathrm{d}^2t}{\mathrm{d}r^2}+\frac{1}{r}\frac{\mathrm{d}t}{\mathrm{d}r}+\frac{\dot{Phi}}{\lambda}=0$$

根据对称性条件 $\frac{\mathrm{d}t}{\mathrm{d}x}\big|_{x=0}=0$ 第一类边界条件 $t(R)=t_w$ 可得圆柱的温度分布为
$$t=\frac{\dot{\Phi}}{4\lambda}(R^2-r^2)+t_w$$
