...
3. 卡诺热机
    ...
    * $$Q_1 = \nu RT_a ln\frac{V_{2}}{V_{1}}$$
    $$Q_2 = \nu RT_b ln\frac{V_{4}}{V_{3}}(Q_2<0, 沿循环方向积分, V_4<V_3)$$
    $$\because(绝热过程的泊松方程)\\ T_aV_2^{\gamma-1}=T_bV_3^{\gamma-1}\\T_aV_1^{\gamma-1}=T_bV_4^{\gamma-1}$$
...
$$\eta _可 = 1 - \frac{T_1}{T_2}=1-\frac{Q_1}{Q_2}$$
规定 $Q_n$ 为过程中从热源吸收的热量, $Q_2$ 取负值(上式中, $Q_2$ 为向低温热源放出的热量, 为正值)
$$\therefore \frac{Q_1}{T_1}+\frac{Q_2}{T_2}=0\to \oint\frac{dQ}{T}=0\to \int_L\frac{dQ}{T}与路径无关$$
$$\eta _{不可}=1 - \frac{Q_1}{Q_2} < 1 - \frac{T_1}{T_2}$$
$$\therefore \frac{Q_1}{T_1}+\frac{Q_2}{T_2}<0\to\oint\frac{dQ}{T}<0\to\int_{L_{不可逆}}\frac{dQ}{T}<\int_{L_{可逆}}\frac{dQ}{T}$$
### 熵增加原理
#### 熵定义
$$dS=\frac{dQ}{T}(T为热源温度)$$
对于==两个状态== $1(p_1,V_1), 2(p_2,V_2)$ ==沿任意可逆过程==有 $$S_2-S_1=\int_1^2\frac{dQ}{T}$$
$$\because \int_{L_{不可逆}}\frac{dQ}{T}<\int_{L_{可逆}}\frac{dQ}{T}$$
$$\therefore \int_{L_{不可逆}}\frac{dQ}{T}<S_2-S_1\to\frac{dQ}{T}<dS(不可逆过程)$$
$\therefore$ 无法直接使用不可逆过程的 $\int_{L_{不可逆}}\frac{dQ}{T}$  计算熵变, 但可以根据不可逆过程的初末状态设计对应的可逆过程, 计算系统的熵变
#### 熵增加
